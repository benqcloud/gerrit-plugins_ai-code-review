...
@Slf4j
public class AIAIChatPromptStateless extends AIChatGptPrompt {
    ...
    public AIAIChatPromptStateless(Configuration config) {
        super(config);
        loadStatelessPrompts();
    }

    public AIAIChatPromptStateless(Configuration config, boolean isCommentEvent) {
    ...
    public String getAISystemPrompt() {
        List<String> prompt = new ArrayList<>(Arrays.asList(
                ...
                AIAIChatPromptStateless.DEFAULT_GPT_SYSTEM_PROMPT_INPUT_DESCRIPTION
        ));
        if (!isCommentEvent) {
            prompt.add(AIAIChatPromptStateless.DEFAULT_GPT_SYSTEM_PROMPT_INPUT_DESCRIPTION_REVIEW);
        }
    ...
    public String getGptUserPrompt(ChangeSetData changeSetData, String patchSet) {
        ...
        if (isCommentEvent && isValidRequestDataPrompt) {
            ...
            prompt.add(AIAIChatPromptStateless.DEFAULT_GPT_REVIEW_PROMPT);
            prompt.addAll(getReviewSteps());
            prompt.add(AIAIChatPromptStateless.DEFAULT_GPT_REVIEW_PROMPT_DIFF);
            prompt.add(patchSet);
            if (isValidRequestDataPrompt) {
                prompt.add(AIAIChatPromptStateless.DEFAULT_GPT_REVIEW_PROMPT_MESSAGE_HISTORY);
